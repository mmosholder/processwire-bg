...
$('[data-recurme-holder="true"]').each(function(){
	...
	function readRule( rrule ) {
		...
		if (recur.FREQ && recur.DTSTART) {
		    ...
		    // Setup start-date picker 2017 12 27 T 06 00 00 Z
	    	    startYear 	= recur.DTSTART.substring(0,4);
	    	    startMonth 	= recur.DTSTART.substring(4,6);
	    	    startDay 	= recur.DTSTART.substring(6,8);
	    	    startHour	= recur.DTSTART.substring(9,11);
	    	    startMin	= recur.DTSTART.substring(11,13);
	    	    startSec	= recur.DTSTART.substring(13,15);
	    	    //startAMPM	= recur.DTSTART.substring(17,19)
	    	    //startAMPM 	= (startHour >= 12) ? "pm" : "am";
	    	    if(startHour == 24){ startAMPM = 'am'; }
	    	    if(startHour == 12){ startAMPM = 'pm'; }
	    	    if(startHour > 12){  startAMPM = 'pm'; }
	    	    if(startHour < 12){  startAMPM = 'am'; }
	    	    
	    	    // fix 24 hour time
	    	    if(startHour > 12){ startHour = eval(parseInt(startHour)-12);}
	    	    if(startHour < 10){ startHour = (parseInt(startHour)).pad(); }
	    	    
		    var startDateString = startYear + '-' + startMonth + '-' + startDay+' '+startHour+':'+startMin+' '+startAMPM;
		    recurme.find('.start-date-field').val(startDateString);
		    recurme.find('.start-date-hidden').val(startYear+startMonth+startDay + 'T'+startHour+startMin+startSec+'Z');
		    ...
		    var startDate = recurme.find('.start-date-field').datetimepicker({
	      		...
	      		onSelect: function(value) {
		      		// 2017-03-21 06:00 am
			  		dsYear 	= value.substring(0,4);
			  		dsMonth = value.substring(5,7);
			  		dsDay 	= value.substring(8,10);
			  		dsHour 	= value.substring(11,13);
			  		dsMin 	= value.substring(14,16);
			  		dsSec 	= '00';
			  		dsAMPM 	= value.substring(17,19);
			  		
			  		// fix to 24 hour time
			  		if(dsAMPM == 'pm'){ dsHour = eval(parseInt(dsHour)+12);
				  		if(dsHour == 24){ dsHour ='12'; }
				  	}else{
					  	if(dsHour == 12){ dsHour = '00'; }
				  	}
			  		//console.log("AMPM?"+dsAMPM+' hour:'+dsHour);
			  		...
			  		fullDateString = dsYear+dsMonth+dsDay+"T"+dsHour+dsMin+dsSec+"Z";
		    ...
		    recurme.find('.end-date').datepicker({
		  		...
		  		changeYear: true,
	      		onSelect: function(value) {
			    dateSelected = new Date(value + 'T23:59:00Z');
			    untilString = dateSelected.getFullYear() + ('0' + (dateSelected.getMonth()+1)).slice(-2) + ('0' + dateSelected.getDate()).slice(-2);
			    recurme.find('.end-date-hidden').val(untilString + 'T235900Z').trigger('change');
			    // Remove the count variable
			    recurringRule.count = '';
			    // Set until variable
			    recurringRule.until = untilString + 'T235900Z';
			    recurme.find('#rrule').trigger('processRecur');
			}	
		    ...
		    if ( recur.UNTIL ) {
				...
				recurme.find('.end-date').val(endYear + '-' + endMonth + '-' + endDay);
				recurme.find('.end-date-hidden').val(endYear+endMonth+endDay + 'T235900Z').trigger('change');
    ...
    var startDate = recurme.find('.start-date-field').datetimepicker({
  		...
  		onSelect: function(value) {
			...
			//$('#start-date-hidden').val(dtstartString + 'T060000z').trigger('change');
		    recurme.find('#rrule').trigger('processRecur');
		}
	});
	...
	recurme.find('.end-date').datepicker({
		...
		dateSelected = new Date(value.replace(/-/g, "/") + ' 23:59:00'); // REGEX used to please SAFARI browser!
		untilString = dateSelected.getFullYear() + ('0' + (dateSelected.getMonth()+1)).slice(-2) + ('0' + dateSelected.getDate()).slice(-2);
		recurme.find('.end-date-hidden').val(untilString + 'T25900Z').trigger('change');
		// Remove the count variable
		recurringRule.count = '';
		// Set until variable
		recurringRule.until = untilString + 'T235900Z';
